...
const createTaskAndRightAnswer = () => {
  const startNumber
    = Math.floor(Math.random() * (MAX_START_PROGRESSION - MIN_START_PROGRESSION + 1))
      + MIN_START_PROGRESSION;
  const step
    = Math.floor(Math.random() * (MAX_STEP_PROGRESSION - MIN_STEP_PROGRESSION + 1))
      + MIN_STEP_PROGRESSION;
  const length
    = Math.floor(Math.random() * (MAX_PROGRESSION_LENGTH - MIN_PROGRESSION_LENGTH + 1))
